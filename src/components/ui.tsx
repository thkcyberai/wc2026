...
// Small shared UI primitives: loading, error and status chips.
...
export function StatusChip({ status }: { status: 'scheduled' | 'live' | 'finished' }) {
  ...
  if (status === 'finished') {
    return <span className="chip bg-zinc-500/20 text-zinc-300">FT</span>;
  }
  return <span className="chip bg-sky-500/20 text-sky-300">Scheduled</span>;
}

export function QualBadge({ status }: { status: string }) {
  switch (status) {
    case 'qualified':
      return <span className="chip bg-accent/20 text-accent">Qualified</span>;
    case 'best-third':
      return <span className="chip bg-gold/20 text-gold">3rd · in race</span>;
    case 'in-contention':
      return <span className="chip bg-sky-500/15 text-sky-300">In contention</span>;
    case 'eliminated':
      return <span className="chip bg-red-500/15 text-red-300">Eliminated</span>;
    default:
